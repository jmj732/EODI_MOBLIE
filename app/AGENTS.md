...
## Purpose
Expo Router 라우트 디렉토리. 파일 경로 = URL 경로. 역할별 탭 그룹 3개(`(user)`, `(teacher)`, `(admin)`)와 인증 플로우(`auth/`)로 구성.
...
| `(teacher)/` | TEACHER 역할 탭 (찾기·상점·지급로그·내 정보) (see `(teacher)/AGENTS.md`) |
...
routeForRole("TEACHER") // → "/(teacher)/point"
